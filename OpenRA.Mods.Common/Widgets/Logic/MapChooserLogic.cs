	...
	public class MapChooserLogic : ChromeLogic
	{
		...
		void SetupGameModeDropdown(MapClassification tab, DropDownButtonWidget gameModeDropdown, ScrollItemWidget itemTemplate)
		{
			if (gameModeDropdown != null)
			{
				...
				var allMapsText = FieldLoader.Translate("MAPCHOOSER-DROPDOWN-ALLMAPS-TEXT");
				Func<Pair<string, int>, string> showItem = x => "{0} ({1})".F(x.First ?? allMapsText, x.Second);
				...
				Func<Pair<string, int>, ScrollItemWidget, ScrollItemWidget> setupItem = (ii, template) =>
				{
					...
					string labelText = showItem(ii);
					item.Get<LabelWidget>("LABEL").GetText = () => labelText;
					return item;
				};
				...
				var dropdownItem = categories.FirstOrDefault(m => m.First == category);
				if (dropdownItem == default(Pair<string, int>))
					dropdownItem.First = FieldLoader.Translate("MAPCHOOSER-DROPDOWN-NOMATCHES-TEXT");

				gameModeDropdown.Text = showItem(dropdownItem);
			}
		}
		...
		void EnumerateMaps(MapClassification tab, ScrollItemWidget template)
		{
			...
			foreach (var loop in maps)
			{
				...
				if (detailsWidget != null)
				{
					...
					details += FieldLoader.Translate("MAPCHOOSER-DETAILS-PLAYERS-TEXT").F(preview.PlayerCount);
				...
				if (authorWidget != null)
				{
					...
					var author = FieldLoader.Translate("MAPCHOOSER-DETAILS-AUTHOR-TEXT").F(preview.Author);
					author = WidgetUtils.TruncateText(author, authorWidget.Bounds.Width, font);
				...
				if (sizeWidget != null)
				{
					...
					if (numberPlayableCells >= 120 * 120) size += FieldLoader.Translate("MAPCHOOSER-DETAILS-SIZE-HUGE-TEXT");
					else if (numberPlayableCells >= 90 * 90) size += FieldLoader.Translate("MAPCHOOSER-DETAILS-SIZE-LARGE-TEXT");
					else if (numberPlayableCells >= 60 * 60) size += FieldLoader.Translate("MAPCHOOSER-DETAILS-SIZE-MEDIUM-TEXT");
					else size += FieldLoader.Translate("MAPCHOOSER-DETAILS-SIZE-SMALL-TEXT");
		...
		void DeleteOneMap(string map, Action<string> after)
		{
			ConfirmationDialogs.ButtonPrompt(
				title: FieldLoader.Translate("MAPCHOOSER-CONFIRM-DELETE-MAP-TEXT"),
				text: FieldLoader.Translate("MAPCHOOSER-CONFIRM-DELETE-TEXT").F(modData.MapCache[map].Title),
				onConfirm: () =>
				{
				...
				confirmText: FieldLoader.Translate("MAPCHOOSER-CONFIRM-DELETE-BUTTON-TEXT"),
				cancelText: FieldLoader.Translate("MAPCHOOSER-CONFIRM-CANCEL-BUTTON-TEXT"),
				onCancel: () => { });
		}
		...
		{
			ConfirmationDialogs.ButtonPrompt(
				title: FieldLoader.Translate("MAPCHOOSER-CONFIRM-DELETEALL-MAP-TEXT"),
				text: FieldLoader.Translate("MAPCHOOSER-CONFIRM-DELETEALL-TEXT"),
				onConfirm: () =>
				{
